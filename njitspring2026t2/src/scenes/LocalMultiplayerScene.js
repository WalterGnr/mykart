...
export default class LocalMultiplayerScene extends RaceScene {   
    ...
    async create() 
    {
        ...
        await super.create();
        // this.keys.accelerate2 = this.input.keyboard.addKey('P'),
    ...
    update() 
    {
        if (!this.clientPlayer || this.localPlayers.length == 0) return;
        ...
        this.localPlayers.forEach(playerObject => {
            if (playerObject.player !== this.clientPlayer){
                ...
                this.playerControllerInputChecks(player, theta, playerObject.gamepad)
